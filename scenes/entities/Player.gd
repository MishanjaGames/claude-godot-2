...
## Currency. Earned by selling items, modified by ShopUI.
var gold: int = 0
...
func _handle_movement(delta: float) -> void:
	...
	if _is_sprinting:
		...
		current_stamina = minf(stat_block.get_max_stamina(),
			current_stamina + stat_block.stamina_regen * delta)
	_emit_stamina()
...
func _melee_attack(weapon: MeleeData) -> void:
	...
	for body in attack_hitbox.get_overlapping_bodies():
		...
		if body.has_method("apply_knockback"):
			body.apply_knockback((body.global_position - global_position).normalized() * weapon.knockback_force)
...
func _ranged_attack(weapon: RangedData) -> void:
	...
	for i in weapon.projectiles_per_shot:
		var angle_offset := 0.0
		if weapon.projectiles_per_shot > 1:
			var spread := deg_to_rad(weapon.spread_angle)
			angle_offset = lerp(-spread * 0.5, spread * 0.5,
				float(i) / float(weapon.projectiles_per_shot - 1))
		var proj: Node2D = weapon.projectile_scene.instantiate()
		get_tree().current_scene.add_child(proj)
		proj.global_position = global_position
		if proj.has_method("setup"):
			proj.setup(base_dir.rotated(angle_offset), weapon.damage + stat_block.get_attack(), weapon.piercing)
...
func _unarmed_attack() -> void:
	...
	for body in attack_hitbox.get_overlapping_bodies():
		if body == self or not body.has_method("take_damage"):
			continue
		body.take_damage(stat_block.get_attack(), "physical", self)
		EventBus.hud_show_popup.emit(str(stat_block.get_attack()), body.global_position, Color.WHITE)
	attack_hitbox.monitoring = false
...
func _apply_on_hit_effects(weapon: WeaponData, target: Node) -> void:
	if weapon.on_hit_effects.is_empty() or randf() > weapon.on_hit_chance:
		return
